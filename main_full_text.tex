\documentclass[25pt,margin=1in,innermargin=-7in,blockverticalspace=0in]{tikzposter}
...
\begin{columns}
    ...
    \block{Abstract}{ \scalefont{1.12}
        ...
        \begin{tikzfigure}[frog model on the $2$-ary tree]
            \includegraphics[width=0.5\linewidth]{tree.png} \vspace{-1\baselineskip}
        \end{tikzfigure}
    }
    ...
    \end{tabular}}.\] For $m>13$ we have approximated upper bounds on $S_m$. See Figure 2 on the right. 
    The horizontal axis is the value of $m$ and the vertical axis is the upper bound we get for $S_m$. We use circles and triangles to denote the rigorous and approximate bounds respectively.

    Establishing these bounds relies heavily on a modified \emph{self-similar frog model} $\SFM(d,p)$. It has several key properties: every vertex now has independent $\Poi(1)$ dormant frogs initially; no backtracking is allowed; only one active frog is ever allowed per subtree; and frogs die upon hitting the root. The self-similar structure and the $\Poi(1)$-per-vertex setup simplify the analysis significantly. % Importantly, let $V_{\,\cdot\,}$ be the number of root visits, then if we have $p$ such that $V_{\SFM(d,p)} = \infty$, then $V_{\FM(d,p)} = \infty$, which implies $p \geq p_d^*$.
    ...
    % \edit{FC - I am in favor of defining U\_m before the sup fdp formula. That way I can explain the algorithm in a sequential order. Also maybe still separate into two columns}
    
    % \edit{EY - not sure about this approach. Readers might be confused when we define the frog star out of nowhere. We should make it clear that the frog star exists for the sole purpose of establishing bounds on the original frog model. }

    % \edit{FC - if we introduce (1) first, then the frog star is coming out of nowhere. The frog star is a model for analyzing the dynamics of the SFM in the first place, from which we find we find (1) gives the desired bounds.}
    % \edit{I am going to look at the slides we had back in August and modify the parts introducing SFM and the frog star. More details should be included than the slides tho, but getting into the details of dynamics would be too much. - FC}
    We remark that the jump probabilities in $\SFM(d,p)$ now depend on two quantities: \[
        p_d^* = \frac{p(d-1)}{d - (d+1)p}\quad \text{and} \quad \hat{p} = \frac{p}{1-p}.
    \]
    The \emph{frog star} introduced in \cite{bailey2023critical} can be viewed as a single self-similar unit of $\SFM$. In the frog star model (Figure 3), there are $\Poi(1)$ active particles at $\varnothing'$ and $\Poi(\lambda)$ active particles at each $v_1,\ldots,v_d$. Initially, the particles at $\varnothing'$ and $v_1$ are active (red), and the rest are dormant (blue). Active particles starting at $\varnothing'$ move to $\varnothing$ with probability $p_d^*$ and otherwise to a uniformly sampled child vertex. Particles at child vertices move to $\varnothing'$ with probability $1$ and then to $\varnothing$ with probability $\hat{p}$, and to $v_1, \ldots, v_d$, except for their starting vertices, with probability $1 - \hat{p}$. Let $U$ denote the number of vertices among $v_2, \ldots, v_d$ that are visited in the end.

    \setlength{\columnsep}{-2em} \setlength{\wrapoverhang}{\marginparwidth}
    \begin{wrapfigure}{r}{0.35\colwidth}
        \vspace{-2em}
            ...
            \end{tikzpicture} \vspace{-.6\baselineskip} % \quad
    ...
    The primary concern is to find the distribution of $U$. In \cite{bailey2023critical} the authors were able to compute its distribution by hand for $d = 2, 3, 4$. In our research we found an inductive way to compute the distribution of $U(d,p,\lambda)$ for higher $d$'s, which can be summarized using Figure 4.

    Under a simple change of variable, our $f(\lambda)$ becomes a polynomial $g(y)$ on $(0,1]$. This reduces our question to finding the unique maximum of the polynomial $g(y)$ and showing that it is less than $1$, which is not hard to implement by computers. We get rigorous bounds on $p_m$ for $2\leq m \leq 13$.
    }
...
% \end{figure}
    ...
    We now show that each bound we attained on $p_m$ ($2\leq m\leq 13$) in the ``Methods'' section is in fact a uniform bound on $p_d$ for all $d \geq m$, as presented in the table in "Improvement". A coupling argument tells us that $\SFM(d,p)$ has stochastically more root visits as $d$ increases. Therefore the bound on $p_m$ we computed makes $\SFM(d,p)$, and hence $\FM(d,p)$, recurrent for all $d \geq m$.
    ...
    \block[bodyverticalshift=-\stdspace]{A follow-up result}{ \scalefont{1.12} \setlength{\parskip}{\stdspace}
        We would like to stress that the computation method presented here has been the only way to achieve upper bounds for $p_d\ (d\geq 3)$. 
